...
def store_results_in_json(json_data): 
    ...
    for data in json_data:
        for result in data['results']:
            post = {}
            address = result['formatted_address'].strip()
            ...
            post['title'] = name.strip()
            post['zipcode'] = zip_code.strip()
            post['street'] = address_split_by_comma[0].strip()
            post['city'] = address_split_by_comma[1].strip()
            post['state'] = state.strip()
            if 'photos' in result: 
                photo_reference = result['photos'][0]['photo_reference'].strip()
